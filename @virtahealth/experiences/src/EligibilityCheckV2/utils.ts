...
export const HEALTH_HISTORY_ENDPOINT = "/applicant_tasks/health_history";
...
export const getPlanName = (coverage: Coverage): string => {
...
export const getGroupId = (coverage: Coverage): string => {
  if (
    coverage &&
    coverage.grouping &&
...
export const getCarrierDropdownOptions = (
  ...
  if (!deploymentPayors?.length || !deployment) {
  ...
  // @ts-ignore: parsed options assumes undefined would be a type but can't be with the filter
